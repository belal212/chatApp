...
import com.example.chatapp.server.ChatHandler;
...
public class Client  {
    ...
    private void setData(){
        ...
        try {
            fis = new FileInputStream("src/main/java/com/example/chatapp/chatroomapplication/server.properties");
            properties.load(fis);
...
//                    String finalServerResponse = serverResponse;
                    Platform.runLater(() ->{
                        new GUIComponents().generateOtherMessageBox(nickname,"/png100px/eg.png", message, v,replayBox);
                    });
                }
            } catch (IOException e) {
                if (running) {
